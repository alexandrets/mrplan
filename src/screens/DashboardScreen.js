...
const MainAppScreen = () => {
  ...
  const [tasks, setTasks] = useState([
    ...
    { id: '4', title: 'Preparar presentación', section: 'today', completed: false, dateCreated: Date.now() },
    { id: '5', title: 'Comprar leche', section: 'today', completed: false, dateCreated: Date.now() },
    { id: '6', title: 'Organizar escritorio', section: 'next', completed: false, dateCreated: Date.now() },
  ...
  // Debug useEffect para monitorear cambios en showAddTask
  useEffect(() => {
    console.log('showAddTask cambió a:', showAddTask);
  }, [showAddTask]);
  ...
  const navigateToGTD = () => {
    console.log('=== NAVIGATE TO GTD CALLED ===');
    console.log('Current slideAnim value:', slideAnim._value);
    console.log('SCREEN_HEIGHT:', SCREEN_HEIGHT);
    console.log('isDashboardVisible before:', isDashboardVisible);
    
    setShowAddTask(false); // Cierra cualquier modal abierto
    setIsDashboardVisible(false);
    
    console.log('Starting animation to:', -SCREEN_HEIGHT);
    Animated.timing(slideAnim, {
      toValue: -SCREEN_HEIGHT,
      duration: 1000, // Increased duration to see the animation
      useNativeDriver: false, // Changed to false for debugging
    }).start((finished) => {
      console.log('Animation completed, finished:', finished);
      console.log('Final slideAnim value:', slideAnim._value);
    });
  };
  ...
  // Función específica para abrir modal desde Dashboard
  const handleAddTaskFromDashboard = () => {
    console.log('=== ADD TASK FROM DASHBOARD ===');
    setNewTaskSection('today');
    setShowAddTask(true);
  };

  // Función para abrir modal desde GTD
  const handleAddTaskFromGTD = () => {
    console.log('=== ADD TASK FROM GTD ===');
    setNewTaskSection('inbox');
    setShowAddTask(true);
  };

  // Agregar nueva tarea - con sección predeterminada
  const handleAddTask = () => {
    if (newTaskTitle.trim()) {
      ...
      setNewTaskTitle('');
      setNewTaskSection('today'); // Reset to 'today'
  ...
  return (
    <>
      <View style={styles.container}>
        ...
        >
          <DashboardScreenOriginal 
            ...
            setTasks={setTasks}
            onNavigateToGTD={() => {
              console.log('=== INLINE NAVIGATE TO GTD CALLED ===');
              alert('GTD Navigation Called!');
              console.log('Current slideAnim value:', slideAnim._value);
              console.log('SCREEN_HEIGHT:', SCREEN_HEIGHT);
              setShowAddTask(false);
              setIsDashboardVisible(false);
      ...
      {/* Modal renderizado fuera de todo */}
      {renderModal()}
    </>
  );
};
