// MainAppScreen.js (versión de depuración para LECTURA y CREACIÓN)
...
const MainAppScreen = () => {
  ...
  // --- EFECTO PARA CARGAR TAREAS EN TIEMPO REAL (CON DEPURACIÓN) ---
  useEffect(() => {
    console.log("--- SE EJECUTA EL EFECTO DE TAREAS ---");
    console.log("El estado del 'user' en este momento es:", user);

    if (!user) {
      console.log("No se ha encontrado un usuario. Limpiando tareas y deteniendo la suscripción.");
      setTasks([]);
      return;
    }

    console.log("Usuario encontrado con UID:", user.uid, ". Creando suscripción a Firestore...");
    
    const unsubscribe = subscribeToUserTasks(user.uid, (fetchedTasks) => {
      console.log("Datos recibidos de Firestore:", fetchedTasks.length, "tareas.");
      setTasks(fetchedTasks);
    });

    return () => {
      console.log("Limpiando la suscripción a tareas.");
      unsubscribe();
    };
  }, [user]);


  // --- FUNCIONES MODIFICADAS PARA USAR FIREBASE (CON DEPURACIÓN) ---
  const handleAddTask = () => {
    if (newTaskTitle.trim() && user) {
      const newTaskData = {
        title: newTaskTitle.trim(),
        section: newTaskSection,
        userId: user.uid, // ¡Este es el campo clave!
      };

      // --- LÍNEA DE DEPURACIÓN PARA LA CREACIÓN ---
      console.log("Intentando crear tarea con estos datos:", newTaskData);
      // ---------------------------------------------
  ...
  // --- EL RESTO DE FUNCIONES DE NAVEGACIÓN Y UI SE MANTIENEN IGUAL ---
          ...
          return (
            <View style={[styles.taskContent, { opacity }]}>
              <TouchableOpacity 
                ...
                onPress={() => completeTask(task.id, task.completed)}
              >
    ...
    <>
      <View style={styles.container}>
        <SafeAreaView style={styles.gtdContainer}>
          ...
          >
            <TaskSection title="Inbox" sectionKey="inbox" tasks={getTasks('inbox')}/>
            <TaskSection title="Today" sectionKey="today" tasks={getTasks('today')}/>
            <TaskSection title="Next" sectionKey="next" tasks={getTasks('next')}/>
            <TaskSection title="Someday" sectionKey="someday" tasks={getTasks('someday')}/>
          </ScrollView>
          <TouchableOpacity 
            style={styles.gtdAddButton}
            onPress={handleAddTaskFromGTD}
          >
      ...
      >
        <View style={styles.modalOverlay}>
            <TouchableOpacity style={StyleSheet.absoluteFill} onPress={() => setShowAddTask(false)} />
...
// Tus estilos (styles) van aquí sin cambios...
