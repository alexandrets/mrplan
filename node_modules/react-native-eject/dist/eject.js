...
const copyProjectTemplateAndReplace_1 = __importDefault(require("./copyProjectTemplateAndReplace"));
/**
...
function eject() {
    ...
    const is75OrAbove = semver_1.default.lt('0.75.0', semver_1.default.coerce(version));
    if (is75OrAbove) {
        const doesTemplateExist = fs_1.default.existsSync(path_1.default.resolve('node_modules/@react-native-community/template'));
        if (!doesTemplateExist) {
            cli_tools_1.logger.error('You need to install `@react-native-community/template@' + version + '` before ejecting.');
            process.exit(1);
        }
        const templateVersion = semver_1.default.coerce(pkgJson.dependencies['@react-native-community/template'] ?? pkgJson.devDependencies['@react-native-community/template']);
        if (!semver_1.default.eq(version, templateVersion)) {
            cli_tools_1.logger.error('Version mismatch detected! Your `react-native` version is ' +
                version +
    ...
    if (!doesIOSExist) {
        ...
        (0, copyProjectTemplateAndReplace_1.default)(path_1.default.resolve('node_modules', is75OrAbove ? '@react-native-community/template' : 'react-native', 'template', 'ios'), path_1.default.resolve('ios'), appName, templateOptions);
    }
    if (!doesAndroidExist) {
        cli_tools_1.logger.info('Generating the Android folder.');
        (0, copyProjectTemplateAndReplace_1.default)(path_1.default.resolve('node_modules', is75OrAbove ? '@react-native-community/template' : 'react-native', 'template', 'android'), path_1.default.resolve('android'), appName, templateOptions);
    }
}
exports.default = {
    name: 'eject',
